...
function parseSchedule() {
	...
	const scheduleCarrier = [
		...
		[] //searchGrid: { terms, djIds } for cells pinned to quoted search terms,
		   //null for every other cell (see lib/schedule/cellPin.js)
	]
	...
	Cell conventions (see id-lookup.js):
	  ...
	  '<anything> | "text"'         -> click-through searches show names for "text"
	                                   instead of linking to one DJ, e.g.
	                                   '[SP] Local Music Hour | "local music hour"'.
	  '<anything> | "a", "b", <id>' -> searches for EITHER term and folds in that
	                                   DJ's shows too. Quoted = search term, bare
	                                   number = DJ id; mix freely.
	*/
