...
function buildFirstLFromBackupCell(fullNameGridBackup, rowIndex, colIndex) {
    ...
    // Drop a trailing pin (e.g. "Tremonti, Matthew | 934", or a quoted search
    // pin) first, so the last-initial comes from the name and not the pin —
    // otherwise the id gets glued into the first name and we render
    // "Matthew | 934 T".
    sourceValue = stripPin(sourceValue)
