...
[5] displayOverrideGrid   (custom display names for [X]/[SP] cells)
[6] searchGrid            ({ terms, djIds } for cells pinned to search terms, else null)
*/

import { splitPinnedId } from "./cellPin";
...
export async function lookupIDsfromFullNames(scheduleCarrier) {
    ...
    for (const row of fullNameGrid) {
        for (const cell of row) {
            const { base, pinnedId, searchTerms } = splitPinnedId(cell);
            if (!base || pinnedId !== null || searchTerms.length) {
                continue;
            }
    ...
    const displayResult = [];
    // search pins: { terms, djIds } for cells that named quoted search terms,
    // null everywhere else. WeeklySchedule links these to /search instead of /dj.
    const searchResult = [];
    ...
    for (const [rowIndex, row] of fullNameGrid.entries()) {
        ...
        for (const [colIndex, cell] of row.entries()) {
            ...
            if (!base && pinnedId === null && searchTerms.length === 0) {
                idRow.push(OTTO_ID);
            ...
            // a term pin sends the cell to /search; any ids alongside it ride
            // along as extra sources rather than becoming a plain /dj link
            searchRow.push(
                searchTerms.length ? { terms: searchTerms, djIds: pinnedId } : null
            );
    ...
    scheduleCarrier[6] = searchResult;
