...
// Free-text sub-genre for display ("Horror/Narration", "Japanese citypop").
// Returns "" when it adds nothing: empty, the same default the `genre` category
// holds (most shows store "Playlist" in both), or already shown as the title.
// Pass `displayedTitle` when the caller may have used othergenre as the title —
// PlaylistView falls back to it for untitled shows, showTitleOrDefault doesn't.
export function showSubGenre(show = {}, displayedTitle) {
    const subGenre = (show.othergenre || "").trim();
    if (!subGenre) return "";
    if (subGenre === (show.genre || "").trim()) return "";
    if (displayedTitle && subGenre === displayedTitle.trim()) return "";
    return subGenre;
}
